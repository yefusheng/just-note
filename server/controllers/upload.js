...
async function parseAppAndInsertToDB(file, user, team) {
   ...
   // 空间对应的机房
    ...
    //上传到七牛后保存的文件名
    var key = fileName+ path.extname(filePath);
    //构建上传策略函数
    ...
    function uploadFile(uptoken, key, localFile,_putExtra,method) {
        return new Promise(function(resolve,reject){
            var formUploader = new qiniu.form_up.FormUploader(configQiniu);
            var putExtra = new qiniu.form_up.PutExtra();
            if(_putExtra && _putExtra.mimeType){
                putExtra.mimeType = _putExtra.mimeType;
            }

            if(method && method == 'put'){
                formUploader.put(uptoken, key, localFile, putExtra, function(respErr, respBody, respInfo) {
                    if (respErr) {
                        throw respErr;
                    }
    ...
    //调用uploadFile上传
    var  uploadresult =  await uploadFile(token, key, filePath);
    console.log('uploadresult',uploadresult);
    if(!uploadresult.key) {
        throw (new Error("上传七牛失败."))
    }
    //end upload qiniu
    var icon =null;
    if (os.type() != 'Windows_NT') {
        icon = await extractor(filePath, fileName, team);
    }
    ...
    // await fs.renameSync(filePath, fileRealPath)//不存在服务器，只存是cdn
    info.downloadUrl = 'https://eftcdn.exexm.com/'+ uploadresult.key
    info.downloadCdnUrl = 'https://eftcdn.exexm.com/'+ uploadresult.key
    var  plistUploadresult = null;
    ...
    if (!app) {
        ...
        info.icon =icon? path.join(uploadPrefix, icon.fileName):'';
        ...
        app.currentVersionName = info.versionStr
        await app.save()
        info.uploader = user.username;
        info.uploaderId = user._id;
        info.size = file.size//fs.statSync(fileRealPath).size
        var version = Version(info)
        version.appId = app._id;
        if (app.platform == 'ios') {
            // upload plist to cdn ,change mapInstallUrl
             async function uploadPlist(app, version){
                if (!app) {
                ...
                var url = version.downloadCdnUrl;
                ...
                var plistUploadKey = version.appId +'/'+ version.id + '.plist';
                var  plistUploadToken = uptoken(bucket, plistUploadKey);
                var  _plistUploadresult =  await uploadFile(plistUploadToken, plistUploadKey, rendered,{mimeType:'text/xml'},'put');
                console.log('plist uploadresult',_plistUploadresult);
                // if(!_plistUploadresult.key) {
                //     throw (new Error("plist 上传七牛失败."))
                // }
                return _plistUploadresult;
            }
            plistUploadresult= await uploadPlist(app,version);
            if(!plistUploadresult.key) {
                // throw (new Error("plist 上传七牛失败.使用降级方案"))
            }
            version.installUrl = mapInstallUrl(app.id, version.id, plistUploadresult && plistUploadresult.key);
        } else {
            version.installUrl = info.downloadCdnUrl
        }
        await version.save()
...
///映射可安装的app下载地址
function mapInstallUrl(appId, versionId,cdn) {
    if(cdn){
        return `itms-services://?action=download-manifest&url=https://eftcdn.exexm.com/${appId}/${versionId}.plist`
    }
