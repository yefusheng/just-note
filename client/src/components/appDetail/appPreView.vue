...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted() {
      ...
      // 判断是否是手机设备
      if (this.isIos || this.isAndroid||isMobile) {
        this.isPhone = true
      } else {
        this.isPhone = false
      }
      ...
      getAppInfo(shortUrl) {
        AppResourceApi.getAppInfoByShortUrl(shortUrl).then((res) => {
          ...
          if (res.data.version === null) {
              this.$message.error('未检测到版本信息，是否发布过？')
              return
          }
          ...
          this.appVersionInfo.creatDateStr = utils.dateFtt('yyyy-MM-dd hh:mm:ss',releaseDate);
...
//            `itms-services://?action=download-manifest&url=${this.axios.defaults.baseURL}api/plist/${this.appBaseData._id}/${this.appVersionInfo._id}`
          ...
          a.click()
          this.$message.success('正在安装，请按Home在桌面查看')
        } else {
          //将下载代码提前，防止下载完成事件监听不到
          AppResourceApi.downloadedCount(this.appBaseData._id, this.appVersionInfo._id).then(() => {
          }, reject => {
          ...
          const a = document.createElement('a')
          let url = this.appVersionInfo.downloadCdnUrl
